...
def test_row_menus_are_keyboard_navigable(seeded):
    """The ⋯ menus carry the roles and hooks rowMenu() drives.

    The key handling itself is in app.js; this guards the markup it needs,
    which is generated by the shared actions_menu macro.
    """
    html = seeded.test_client().get("/categories/").get_data(as_text=True)
    assert 'x-data="rowMenu()"' in html
    assert 'role="menu"' in html
    assert 'aria-haspopup="menu"' in html
    assert 'x-ref="trigger"' in html and 'x-ref="menu"' in html
    assert "keydown.down.prevent" in html, "arrow keys must open and move"
    assert "keydown.escape" in html, "escape must close and hand focus back"


def test_every_row_menu_uses_the_shared_component(seeded):
    """One implementation, so a11y fixes land everywhere at once."""
    client = seeded.test_client()
    for path in ("/accounts/", "/categories/", "/funds/",
                 "/savings/?tab=mf", "/savings/?tab=deposits"):
        html = client.get(path).get_data(as_text=True)
        triggers = html.count('aria-haspopup="menu"')
        assert triggers > 0, f"{path} has no row menus"
        assert html.count('x-data="rowMenu()"') == triggers, \
            f"{path} still has a hand-rolled menu"
